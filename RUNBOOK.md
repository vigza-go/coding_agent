...
5. （可选）写规则文件：全局 `~/.coding_agent/AGENTS.md`、项目 `<workspace_root>/AGENTS.md`。
   启动时各读一次，之后改文件要重启才生效；`/status` 里的"规则文件"一行会列出实际加载了哪几份、
   各占多少 token，没有就写"无"。
...
模型自己用 `todo` 工具维护一份计划（每次提交完整列表），`/status` 里能看到当前那份。同一时刻
只允许一项 `in_progress`；项数上限 20、标题一行不超过 200 字；不合法就返回 `Error:` 文本，
不会写一半。收尾时若还有没交代的项，框架递**一次**提醒让它再做一轮（只一次，免得陷进死循环）。

计划和 work state 一起，在历史被剪裁/压缩的那一刻贴成便签回灌；平时不每轮注入。
...
- `/thread ID`：切换 thread。目标正被另一个会话跑 turn 时**拒绝切换**，
  以免两边的历史搅在一起。
- `/status`：查看当前 head、记忆块层级时间线、压缩区/工作区 token 占用、当前计划（勾选清单）、
  启动时生效的规则文件（路径 + 体积）和 work state。
...
**输入框下面那行（底栏）** 实时显示轮次水位和当前计划：

```
 t1  ·  head 12  ·  next 13  ·  计划 1/3 · 进行中 写代码
```

- 两个数字就是 `/status` 里的 `active head` / `next user seq`——要 `/undo` 到第几轮时不用再敲命令。
- 计划段只在有 todo 时出现：在做的那项优先（`进行中 …`），手头空了显示 `下一步 …`，全部交代了显示 `全部交代`。
- **回车之后底栏会消失，整轮跑完才回来**：它和输入框是同一个 prompt 界面，提交时一起被收掉。所以这一轮进行中，计划进度挂在进度行上（`正在执行 bash… · 计划 1/3 · 进行中 写代码`），模型每次写完计划就刷新一次——不然整轮都是盲区，只能等底栏回来才知道做到哪了。
- **刷新粒度**：底栏在每次回到输入框之前算一次；进度行只在模型写计划时重算一次（其余事件复用同一份，不给键盘上的每次事件都挂一次查库）。
- 底栏出现 `状态不可用：XXX` 表示读会话状态失败了（会话本身还能用，`/status` 可以再看一眼）。
边框颜色。
