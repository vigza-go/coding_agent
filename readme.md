...
```
核心代码约 5.5k 行 ｜ 测试约 5.5k 行 / 234 个 pytest 用例 ｜ 设计与推导见 DESIGN.md、RUNBOOK.md
```
...
表里这些 token 数都按"模型在这条消息里读得到的字"量（正文、思考块、工具调用参数、工具结果），
不含 JSON 包壳键名，也不含 `artifact`。以前量整包 JSON，等于把触发线提前了 1.3–1.6 倍。
...
**结论是：1M 默认窗口下这个状态不可达，所以宁可作为断言让失败可见，也不静默丢用户记忆。**
⚠️ 但"不可达"只对默认配置成立：把 `total_tokens` 调小后块数跟着变少，实验里 `L/8` 那档就撞到过
`1 个 L3 + 1 个 L0`——不同级、没对可合，直接抛错（见 `experiments/README.md`，那里管它叫
"生产里真会撞到的失败模式"）。
...
累计输入文本量级相当，差别在**重复摘要次数**：分层方案把"摘要的摘要"从线性压成对数级，**省的是成本**。

⚠️ 但这句话不能读成"信息更安全"——恰恰相反。真实生产链上第一次量到逐层后果（09-04 的 44 块快照，一条 L0×8→L1→L2→L3 金字塔）：文件路径逐字保留 100%→12%→5%→**0%**（`file` 类）、`path` 类 100%→14%→5%→**0%**，标识符 100%→42%→15%→**6%**；而**同预算**下去截尾对照，合并产物 16 格里只赢 2 格、输 10 格。分层买到的是每轮少发 token，不是更好的信息保留（全部数字与口径见 `experiments/README.md` 结论三之二）。

**代价也说清楚**：调用次数多、压缩停顿更明显（实测 12 次合并共喂摘要器 94,694 tok，换得此后每轮少发 64,576 tok → 1.47 轮回本；但层内比率只有 2.2–6.2，比 L0 首压的 26.4:1 低一个数量级）。优点是记忆有分辨力。这个取舍是否划算，取决于会话长度——**短会话（不触发压缩）下两者没有差别**，见「已知限制」。

#### 摘要质量控制

单次摘要上限 = **这一段历史在模型上下文里占的量** 的 50%，两条路径同口径：L0 按 `message_tokens()`
量被压缩的那批消息，合并路径以左右两块存的 `token_count` 为基线。这把尺子只数模型读得到的字
（正文、思考块、工具调用参数、工具结果）。改之前它数的是整包 `content_json`：136 个 L0 块中位
27,756，而可见内容只有 17,856 —— 虚高 1.6 倍（键名算钱，工具调用参数还在 `content` 块和
`tool_calls` 里各存一份），算出的上限 13,878 已是原文的 78%，等于没约束。⚠️ 也别反过来用
`sanitize_transcript()` 的输出当分母：那是喂弱摘要器的安全副本，抹掉了思考块、把工具调用压成一行，
只剩可见内容的 37%，拿它算会把名额压掉三分之二。
输出为空或超限时最多重试 3 次，提示词逐次更激进（要求删除解释、重复、可从代码重得的细节，改用极短条目）。3 次都超限则**接受最短的那版超限输出**——上限是软目标，略微超限依然在缩小记忆，只有全空或含协议残骸才抛 `SummaryError`。⚠️ 其中"删除可从代码重新获得的细节"这条升级语（`summarizer.py:56`）实测正是合并层锚点清零的直接原因：它让摘要器**明令不得逐字保留**路径与标识符（另见 `summarizer.py:151`"禁止输出…代码原文"），而 path 类恰是压缩区里占 41% token、下游最常被逐字复用的一类。**修复方向已测过单价但尚未实现**：把锚点原样挂成附录，约 7 tok/条路径、+7% 体积买回 100% 可寻址——实现之前，"合并后仍可寻址"这句话不成立。）

**任一请求最终失败则整批不写 memory block**，记忆原样留在工作区——宁可让这一轮请求偏大，也不静默丢记忆。但**缓存不是原样**：越线那一次的冷思维链剪裁已经写进缓存视图，失败也不回滚（有意如此，见上文 CoT 三条）；库里仍是原文，缓存失效后边界归零。所有摘要通过校验后在一个事务里写入。
...
- **一条 thread 同一时刻只允许一个会话待在里面**：用 MySQL 连接级咨询锁（`GET_LOCK`）实现，不建表、不加字段、没有 TTL。占用是会话级的 —— 启动 `--thread x` 或 `/thread x` 成功即占住，直到你退出或切走；空闲不说话的终端同样挡人。第二个会话进入/切换会被直接拒绝。进程被 `kill -9`、Ctrl-C 或断电，服务端随连接收回锁，下一个会话立刻接管，不需要清锁。会话空闲太久被 `wait_timeout` 收走连接时，下一轮开头自动重取；若这期间已被别人接管则报错而不是静默继续。**不锁工作树**：同一棵树下并行开多个会话（不同 thread）是有意支持的。
...
234 个 pytest 用例，覆盖方向（`uv run pytest -q`，SQLite 即可，无需外部 MySQL）：
...
1. **短任务下压缩基本不激活**。1M 窗口 + 500k 触发线的组合，意味着几十轮以内的普通任务根本不会压缩——分层收益只在长会话、多次压缩后显现。我用受控预算梯度来验证机制，但这不等于生产 1M 下的效果。
   补一条 09-04 的实测：合并**已经在真实长会话里发生了**（07:38–08:04 的 26 分钟内 12 次，块层级到 L3）。但"发生"不等于"有益"——等预算下合并产物的锚点保留率并不优于只留最近的摘要（见上节与 `experiments/README.md` 结论三之二）。
2. **压缩已有真实数据的 token 对照，但仍缺真实摘要器的端到端质量数据**。目前没有"同一任务在不同预算档位下成功率对比"的量化结果，这是下一步最该补的一块。
3. **多进程同一 thread 只能轮流，不能并行**（进入即占用，见第 6 节）。thread 占用锁（MySQL 连接级咨询锁）跨进程有效，能挡住"同时写"，也能挡住"别人切进来";但**读侧仍是进程内缓存**——别的会话提交的轮次不会主动通知我，只能在每次写时间轴前用 seq 分歧检测把已过期的一轮终止掉，或者重启进程。真正的多机部署仍需固定路由或分布式失效通知；SQLite 下没有这把锁。占座者不会因"关了窗口"卡死你：TUI 整条会话期间每秒自检控制终端，终端没了（关标签页只撤销 pty、不发挂断，实测）就先给自己一发 Ctrl-C 走本轮的中断收尾，收尾不配合再硬退；跑 turn 途中关窗口也一样停手。
