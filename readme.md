...
单次摘要硬上限 = 输入 token 的 50%。输出为空或超限时最多重试 3 次，提示词逐次更激进（要求删除解释、重复、可从代码重得的细节，改用极短条目）。⚠️ 其中"删除可从代码重新获得的细节"这条升级语（`summarizer.py:56`）实测正是合并层锚点清零的直接原因：它让摘要器**明令不得逐字保留**路径与标识符（另见 `summarizer.py:151`"禁止输出…代码原文"），而 path 类恰是压缩区里占 41% token、下游最常被逐字复用的一类。**修复方向已测过单价但尚未实现**：把锚点原样挂成附录，约 7 tok/条路径、+7% 体积买回 100% 可寻址——实现之前，"合并后仍可寻址"这句话不成立。）

**任一请求最终失败则整批不写 memory block**，记忆原样留在工作区——宁可让这一轮请求偏大，也不静默丢记忆。所有摘要通过校验后在一个事务里写入。
...
3. **多进程同一 thread 只能轮流，不能并行**（进入即占用，见第 6 节）。thread 占用锁（MySQL 连接级咨询锁）跨进程有效，能挡住"同时写"，也能挡住"别人切进来";但**读侧仍是进程内缓存**——别的会话提交的轮次不会主动通知我，只能在每次写时间轴前用 seq 分歧检测把已过期的一轮终止掉，或者重启进程。真正的多机部署仍需固定路由或分布式失效通知；SQLite 下没有这把锁。
